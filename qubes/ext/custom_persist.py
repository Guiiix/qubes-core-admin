...
class CustomPersist(qubes.ext.Extension):
    """This extension allows to create minimal-state APP with by configuring an exhaustive list of bind dirs
       (and files)
    """
    ...
    def _is_valid_key(key, vm) -> bool:
        if not key:
            vm.log.warning("Got empty custom-persist key, ignoring: {}".format(key))
            return False

        # QubesDB key length limit
        key_maxlen = QDB_KEY_LIMIT - len(QDB_PREFIX)
        if len(key) >= key_maxlen:
            vm.log.warning("custom-persist key is too long (max {}), ignoring: {}".format(
                key_maxlen, key))
            return False
        return True

    def _write_db_value(self, feature, value, vm):
        vm.untrusted_qdb.write("{}{}".format(QDB_PREFIX, self._extract_key_from_feature(feature)), str(value))
    ...
    def on_domain_qdb_create(self, vm, event):
        ...
        for feature, value in vm.features.items():
            if self._is_expected_feature(feature) and self._is_valid_key(self._extract_key_from_feature(feature), vm):
    ...
    def on_domain_feature_delete(self, vm, event, feature):
        ...
        vm.untrusted_qdb.rm("{}{}".format(QDB_PREFIX, self._extract_key_from_feature(feature)))
